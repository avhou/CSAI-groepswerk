...
\begin{itemize}
    \item [\textbf{Faithfulness}] evaluates whether the response stays true to the source materials. This means avoiding new or fabricated information: the answer should not include claims that are not supported by the original context.
    \item [\textbf{Context relevancy}] checks if the answer properly addresses the question based on the text provided. Rather than straying into unrelated details, a relevant response will use the most pertinent information from the reports to respond directly to the query.
